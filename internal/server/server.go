package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-courier/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-courier"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/tasks",s.handleList)
    s.mux.HandleFunc("POST /api/tasks",s.handleCreate)
    s.mux.HandleFunc("PUT /api/tasks/{id}",s.handleUpdate)
