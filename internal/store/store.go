...
func(d *DB)Update(e *Message)error{_,err:=d.db.Exec(`UPDATE messages SET queue=?,payload=?,priority=?,status=?,retries=?,max_retries=?,processed_at=? WHERE id=?`,e.Queue,e.Payload,e.Priority,e.Status,e.Retries,e.MaxRetries,e.ProcessedAt,e.ID);return err}
...
func(d *DB)Search(q string, filters map[string]string)[]Message{
    where:="1=1"
    args:=[]any{}
    if q!=""{
        where+=" AND (1=0)"
    ...
    rows,_:=d.db.Query(`SELECT id,queue,payload,priority,status,retries,max_retries,processed_at,created_at FROM messages WHERE `+where+` ORDER BY created_at DESC`,args...)
    if rows==nil{return nil};defer rows.Close()
    var o []Message;for rows.Next(){var e Message;rows.Scan(&e.ID,&e.Queue,&e.Payload,&e.Priority,&e.Status,&e.Retries,&e.MaxRetries,&e.ProcessedAt,&e.CreatedAt);o=append(o,e)};return o
}

func(d *DB)Stats()map[string]any{
    m:=map[string]any{"total":d.Count()}
    rows,_:=d.db.Query(`SELECT status,COUNT(*) FROM messages GROUP BY status`)
